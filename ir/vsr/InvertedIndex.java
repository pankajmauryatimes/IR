 ...
 *
 * @author Ray Mooney
 * @contributor Tyler Kemme (pseudo relevant feedback)
 */
public class InvertedIndex {
   ...
   */
  public InvertedIndex(File dirFile, short docType, boolean stem, boolean feedback, boolean pseudofeedback, int pseudoDocsNum, double a, double b, double g) {
    this.dirFile = dirFile;
    this.docType = docType;
    this.stem = stem;
    this.feedback = feedback;
    this.pseudofeedback = pseudofeedback;
    this.pseudoDocsNum = pseudoDocsNum;
    this.a = a;
    this.b = b;
    this.g = g;
  ...
  public void presentRetrievals(HashMapVector queryVector, Retrieval[] retrievals) {
    if (showRetrievals(retrievals)) {
      ...
      Feedback pseudofdback = null;
      if (feedback)
        fdback = new Feedback(queryVector, retrievals, this);
      // pseudo feedback
      if (pseudofeedback){
           System.out.println("");
           System.out.println("Processing pseudo relevant feedback with " + pseudoDocsNum + " documents.");
	   pseudofdback = new Feedback(queryVector, retrievals, this);
           pseudofdback.ALPHA = a;
           pseudofdback.BETA = b;
           pseudofdback.GAMMA = g;
           int x = pseudoDocsNum;
           // if pseudoDocsNum is bigger than the number of retrievals, do pseudo feedback with all the retrievals
           if(pseudoDocsNum > retrievals.length){
           pseudoDocsNum = retrievals.length;
               x -= retrievals.length;
           }
           else
             x = 0;
	   // iterate through retrievals and add them to goodDocRefs up to pseudoDocsNum
           for (int i = 0; i < pseudoDocsNum; i++) {
 	       String docName = retrievals[i].docRef.file.getName();
	       pseudofdback.goodDocRefs.add(retrievals[i].docRef);
           }
	  // present new retrievals
          queryVector = pseudofdback.newQuery();
          retrievals = retrieve(queryVector);
          // Update the list of retrievals stored in the feedback
          pseudofdback.retrievals = retrievals;
          while(x > 0){
          if(x > retrievals.length){
	       pseudoDocsNum = retrievals.length;
               x -= retrievals.length;
           }
          else{
	  ...
	  // present new retrievals
          ...
          if (showRetrievals(retrievals))
            ...
            System.out.println("Uh oh an error happened somewhere at some point");
      }
  ...
  public static void main(String[] args) {
	...
	// Use pseudo relevant feedback
        pseudofeedback = true;
	pseudoDocsNum = Integer.parseInt(args[i+1]);
        i++;
      }
    ...
    InvertedIndex index = new InvertedIndex(new File(dirName), docType, stem, feedback, pseudofeedback, pseudoDocsNum, a, b, g);
    // index.print();
